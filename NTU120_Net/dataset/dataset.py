...
class NTU_RGBD(Dataset):
    ...
    def __init__(self, root_path, opt,
        ...
        self.use_Minkowski = False if opt.model == 'PointNet++' else True
    ...
    def __getitem__(self, idx):
        ...
        for s in range(3):
            ...
            path_points_frame = os.path.join(self.depth_path, path_v, v_name1)
        ...
        if self.use_Minkowski is True:
            points_c[:, :3] = (points_c[:, :3] + .5) * self.voxel_size
...
def sparse_tensor_collation_fn(data_labels):
    points_xyzc, points_rgb1, points_rgb2, points_rgb3, labels, v_names = list(zip(*data_labels))

    coords = [points[:, 0:3] for points in points_xyzc]
    feats = [points[:, 3:] for points in points_xyzc]

    # Create batched coordinates for the SparseTensor input
    bcoords = ME.utils.batched_coordinates(coords)

    bs = len(points_rgb1)
    rgb = points_rgb1[0].shape[1]
    # Concatenate all lists
    feats_batch = torch.from_numpy(np.concatenate(feats, 0)).float()
    rgb1_batch = torch.reshape(torch.from_numpy(np.concatenate(points_rgb1, 0)).float(), (bs, -1, rgb))
    rgb2_batch = torch.reshape(torch.from_numpy(np.concatenate(points_rgb2, 0)).float(), (bs, -1, rgb))
    rgb3_batch = torch.reshape(torch.from_numpy(np.concatenate(points_rgb3, 0)).float(), (bs, -1, rgb))
    labels_batch = torch.from_numpy(np.concatenate([labels], 0)).long()

    return (bcoords, feats_batch), rgb1_batch, rgb2_batch, rgb3_batch, labels_batch, v_names
